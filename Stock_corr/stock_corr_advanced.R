...
batch_analysis <- function(tickers, years_selected) {
  ...
  for(i in 1:length(tickers)) {
    ...
    if(!is.null(stock_data)) {
      # Calculate stock returns, continuously log compounded
      ...
      stock_correlations <- numeric(length(benchmarks))
      
      for(j in 1:length(benchmarks)) {
        if(!is.null(benchmark_returns[[j]])) {
          # Data Alignment: Find overlapping trading days
...
if(length(args) >= 2 && args[1] == "--batch") {
  ...
  # Run batch analysis
  batch_analysis(tickers, years_selected)
  quit()
}
